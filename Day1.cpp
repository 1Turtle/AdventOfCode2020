...
vector<int> part1(void) {
    ...
    for(const auto& value1: input) {    for(const auto& value2: input) {
            ...
            if( (value1+value2 == 2020) && (find(result.begin(), result.end(), (value1*value2)) == result.end()) ) {
...
vector<int> part2(void) {
    /* Each for loop compares its numbers. */
    for(const auto& value1: input) {    for(const auto& value2: input) {    for(const auto& value3: input) {
                /* if the result does not exist yet, then it will be pushed into the vector. */
                if( (value1+value2+value3 == 2020) && (find(result.begin(), result.end(), (value1*value2*value3)) == result.end()) ) {
...
int main(void) {
    ...
    for(const auto& r: part1()) cout << "Part 1: " << r << '\n';
    result.clear();
    for(const auto& r: part2()) cout << "Part 2: " << r << '\n';
